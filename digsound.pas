...
hwo := 0;
{$else}
if not ASChecked(snd_pcm_close(handle)) then
 Result := Ord(digsound_rc_error_driver);
if not canpause then //strange feature in Rosa 12.2 in Oracle VM - pause not supporting and snd_pcm_close return little earlier than device closed really
 Sleep(100);
handle := nil;
FreeMem(ufds);
ClearBuffers;
...
{$else}
if canpause then
 ok := ASChecked(snd_pcm_pause(handle,1))
else //todo better
 ok := ASChecked(snd_pcm_drop(handle));
if not ok then
{$endif}
 Result := Ord(digsound_rc_error_driver);
end;
...
{$else}
if canpause then
 ok := ASChecked(snd_pcm_pause(handle,0))
else //todo better
 ok := ASChecked(snd_pcm_prepare(handle));
if not ok then
{$endif}
 Result := Ord(digsound_rc_error_driver);
end;
