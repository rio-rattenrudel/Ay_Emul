...
** ECX: Primary data
** EDX: Primary address
** EBP/RBP: Current base offset of PC
** ESI/RSI: Current PC, including base offset
** EDI: Cycle counter
**
** x64:
** R12: __jmptbl
...
static int x64		   = -1;
static int win64	   = -1;
static int stackalign  = -1;
static char *sourcename = NULL;

#define IS_WIN64 (x64 == 1 && win64 == 1)
#define IS_AMD64 (x64 == 1 && win64 == 0)
...
static char *x86si	 [5] = {"?", "sil" , "si"  , "?", "esi"	 };
static char *sizename[5] = {"?", "byte", "word", "?", "dword"};
static int quickvalue[8] = {8, 1, 2, 3, 4, 5, 6, 7};
static char direction[2] = {'r','l'};
static char *x64rax	 [2] = {"eax", "rax"};
static char *x64rbx	 [2] = {"ebx", "rbx"};
static char *x64rcx	 [2] = {"ecx", "rcx"};
static char *x64rdx	 [2] = {"edx", "rdx"};
static char *x64rbp	 [2] = {"ebp", "rbp"};
static char *x64rsi	 [2] = {"esi", "rsi"};
static char *x64rdi	 [2] = {"edi", "rdi"};
static char *x64rsp	 [2] = {"esp", "rsp"};
static char *defp	 [2] = {"defd", "defq"};

#define REGARG1 (x64 ? (win64 ? "rcx" : "rdi") : "eax")
#define REGARG2 (x64 ? (win64 ? "rdx" : "rsi") : "edx")
#define REGARG3 (x64 ? (win64 ? "r8"  : "rdx") : "ebx")
...
static void optiondump(FILE *o, char *prefix) {
	...
	fprintf(o, "%sStack alignment: %s\n", prefix,
		stackalign ? "on" : "off");
}

static void gen_banner(void) {
	emit("; Generated by STARSCREAM version " VERSION "\n");
	emit("; For assembly by %s only\n", x64 ? "YASM" : "NASM or YASM");
	emit(";\n");
	emit("; Options:\n");
	optiondump(codefile, "; *  ");
	emit(";\n");
...
static void gen_macros() {
	/* alternative struct declaring, YASM is annoying */
	emit("%%macro defstruct 1\n");
	emit("%%1 equ 0\n");
	emit("%%assign %%1_size 0\n");
	emit("%%endmacro\n");
	emit("%%macro def 3\n");
	emit("%%1_%%2 equ %%1_size\n");
	emit("%%assign %%1_size %%1_size+%%3\n");
	emit("%%endmacro\n");
	emit("%%macro defb 3\n");
	emit("def %%1,%%2,%%3*1\n");
	emit("%%endmacro\n");
...
/* registers to preserve according to calling convention */
#define PRESERVE_REGMASK (x64 ? (win64 ? (rBX|rBP|rSI|rDI|r12|r13|r14|r15) : (rBX|rBP|r12|r13|r14|r15)) : (rBX|rBP|rSI|rDI))

/* registers in use throughout starscream */
#define REGSUSED (r8 | r9 | r12 | r13 | rAX | rBX | rCX | rDX | rBP | rSI | rDI)
...
static int preserve(enum Registers regs) {
	int r = 0;
	regs &= PRESERVE_REGMASK;
	if(regs & rAX) { emit("push xAX\n"); ++r; }
	if(regs & rBX) { emit("push xBX\n"); ++r; }
	if(regs & rCX) { emit("push xCX\n"); ++r; }
	if(regs & rDX) { emit("push xDX\n"); ++r; }
	if(regs & rBP) { emit("push xBP\n"); ++r; }
	if(regs & rSI) { emit("push xSI\n"); ++r; }
	if(regs & rDI) { emit("push xDI\n"); ++r; }
	if(regs & r12) { emit("push r12\n"); ++r; }
	if(regs & r13) { emit("push r13\n"); ++r; }
	if(regs & r14) { emit("push r14\n"); ++r; }
	if(regs & r15) { emit("push r15\n"); ++r; }
	return r * (x64 ? 8 : 4);
}

static void restore(enum Registers regs) {
	regs &= PRESERVE_REGMASK;
	if(regs & r15) emit("pop r15\n");
	if(regs & r14) emit("pop r14\n");
	if(regs & r13) emit("pop r13\n");
	if(regs & r12) emit("pop r12\n");
	if(regs & rDI) emit("pop xDI\n");
	if(regs & rSI) emit("pop xSI\n");
	if(regs & rBP) emit("pop xBP\n");
	if(regs & rDX) emit("pop xDX\n");
	if(regs & rCX) emit("pop xCX\n");
	if(regs & rBX) emit("pop xBX\n");
	if(regs & rAX) emit("pop xAX\n");
}
...
static void common_dax(const char* ins)
{
	int myline = linenum; linenum += 4;
	emit("lahf\n");
	emit("jc dax%d\n", myline);
	emit("mov r9b,byte 0\n");
	emit("cmp al,99h\n");
...
static void daa() {
	if(x64) common_dax("add");
	else	emit("daa\n");
}

static void das() {
	if(x64) common_dax("sub");
	else	emit("das\n");
}
...
static void gen_variables(void) {
	/* programregion */
	emit("defstruct pr\n");
	emit("defd pr,lowaddr,1\n");
	emit("defd pr,highaddr,1\n");
	emit("%s   pr,offset,1\n", x64 ? "defq" : "defd");
	
	/* dataregion */
	emit("defstruct dr\n");
	emit("defd dr,lowaddr,1\n");
	emit("defd dr,highaddr,1\n");
	emit("%s   dr,memorycall,1\n", x64 ? "defq" : "defd");
	emit("%s   dr,userdata,1\n", x64 ? "defq" : "defd");
	
	emit("section .data\n");
	emit("bits %d\n", x64 ? 64 : 32);
	emit("global _%scontext\n", sourcename);
	emit("global %scontext\n", sourcename);
	emit("defstruct cpucontext\n");
	/*
	** CONTEXTINFO_MEM16
	** CONTEXTINFO_MEM16FC
	**
	...
	*/
	if(cputype <= 68010) {
		emit("%s cpucontext,fetch,		 1\n", defp[x64]);
		emit("%s cpucontext,readbyte,	 1\n", defp[x64]);
		emit("%s cpucontext,readword,	 1\n", defp[x64]);
		emit("%s cpucontext,writebyte,	 1\n", defp[x64]);
		emit("%s cpucontext,writeword,	 1\n", defp[x64]);
		emit("%s cpucontext,s_fetch,	 1\n", defp[x64]);
		emit("%s cpucontext,s_readbyte,	 1\n", defp[x64]);
		emit("%s cpucontext,s_readword,	 1\n", defp[x64]);
		emit("%s cpucontext,s_writebyte, 1\n", defp[x64]);
		emit("%s cpucontext,s_writeword, 1\n", defp[x64]);
		emit("%s cpucontext,u_fetch,	 1\n", defp[x64]);
		emit("%s cpucontext,u_readbyte,	 1\n", defp[x64]);
		emit("%s cpucontext,u_readword,	 1\n", defp[x64]);
		emit("%s cpucontext,u_writebyte, 1\n", defp[x64]);
		emit("%s cpucontext,u_writeword, 1\n", defp[x64]);
		if(cputype == 68010) {
			emit("%s cpucontext,f_readbyte,	 1\n", defp[x64]);
			emit("%s cpucontext,f_readword,	 1\n", defp[x64]);
			emit("%s cpucontext,f_writebyte, 1\n", defp[x64]);
			emit("%s cpucontext,f_writeword, 1\n", defp[x64]);
		}
	/*
	** CONTEXTINFO_MEM32
	**
	...
	*/
		...
		emit("%s cpucontext,fetch,		 1\n", defp[x64]);
		emit("%s cpucontext,readbus,	 1\n", defp[x64]);
		emit("%s cpucontext,writebus,	 1\n", defp[x64]);
		emit("%s cpucontext,s_fetch,	 1\n", defp[x64]);
		emit("%s cpucontext,s_readbus,	 1\n", defp[x64]);
		emit("%s cpucontext,s_writebus,	 1\n", defp[x64]);
		emit("%s cpucontext,u_fetch,	 1\n", defp[x64]);
		emit("%s cpucontext,u_readbus,	 1\n", defp[x64]);
		emit("%s cpucontext,u_writebus,	 1\n", defp[x64]);
		emit("%s cpucontext,f_readbus,	 1\n", defp[x64]);
		emit("%s cpucontext,f_writebus,	 1\n", defp[x64]);
	}
	/*
	** CONTEXTINFO_COMMON
	**
	...
	*/
	if(cputype >= 68000) {
		emit("%s cpucontext,resethandler,1\n", defp[x64]);
		emit("def  cpucontext,reg,		   0\n");
		emit("defd cpucontext,dreg,		   8\n");
		emit("defd cpucontext,areg,		   7\n");
		emit("defd cpucontext,a7,		   1\n");
		emit("defd cpucontext,asp,		   1\n");
		emit("defd cpucontext,pc,		   1\n");
		emit("defd cpucontext,odometer,	   1\n");
		/* Bit 0 of __interrupts = stopped state */
		emit("defb cpucontext,interrupts,  8\n");
		emit("defw cpucontext,sr,		   1\n");
	}
	/*
	** CONTEXTINFO_68000SPECIFIC
	*/
	if(cputype == 68000) {
		emit("defw cpucontext,contextfiller00,1\n");
	}
	/*
	** CONTEXTINFO_68010
	**
	...
	*/
	if(cputype >= 68010) {
		emit("defb cpucontext,sfc,		   1\n");
		emit("defb cpucontext,dfc,		   1\n");
		//?? why not emit(".contextfiller01	 resw 1\n");
		emit("defd cpucontext,vbr,		   1\n");
		emit("%s cpucontext,bkpthandler, 1\n", defp[x64]);
	}
	/*
	...
	*/
	if(cputype == 68010) {
		emit("defb cpucontext,loopmode,	   1\n");
		emit("defb cpucontext,contextfiller10,3\n");
	}
	/*
	** CONTEXTINFO_68020
	**
...
/*	align(4);*/
	...
	emit("%scontext:\n", sourcename);
	emit("contextbegin:\n");
	emit("times cpucontext_size db 0\n");
	
	if(x64) {
		emit("%%define __ctx r13\n");
		emit("%%define ctx(x) contextbegin + cpucontext_ %%+ x wrt rip\n");
	} else {
		emit("%%define __ctx contextbegin\n");
		emit("%%define ctx(x) contextbegin+cpucontext_ %%+ x\n");
	}
	
	/* Handy defines for easy access */
	if(cputype <= 68010) {
		emit("%%define __fetch		  __ctx+cpucontext_fetch\n");
		emit("%%define __readbyte	  __ctx+cpucontext_readbyte\n");
		emit("%%define __readword	  __ctx+cpucontext_readword\n");
		emit("%%define __writebyte	  __ctx+cpucontext_writebyte\n");
		emit("%%define __writeword	  __ctx+cpucontext_writeword\n");
		emit("%%define __s_fetch	  __ctx+cpucontext_s_fetch\n");
		emit("%%define __s_readbyte	  __ctx+cpucontext_s_readbyte\n");
		emit("%%define __s_readword	  __ctx+cpucontext_s_readword\n");
		emit("%%define __s_writebyte  __ctx+cpucontext_s_writebyte\n");
		emit("%%define __s_writeword  __ctx+cpucontext_s_writeword\n");
		emit("%%define __u_fetch	  __ctx+cpucontext_u_fetch\n");
		emit("%%define __u_readbyte	  __ctx+cpucontext_u_readbyte\n");
		emit("%%define __u_readword	  __ctx+cpucontext_u_readword\n");
		emit("%%define __u_writebyte  __ctx+cpucontext_u_writebyte\n");
		emit("%%define __u_writeword  __ctx+cpucontext_u_writeword\n");
		if(cputype == 68010) {
			emit("%%define __f_readbyte	  __ctx+cpucontext_f_readbyte\n");
			emit("%%define __f_readword	  __ctx+cpucontext_f_readword\n");
			emit("%%define __f_writebyte  __ctx+cpucontext_f_writebyte\n");
			emit("%%define __f_writeword  __ctx+cpucontext_f_writeword\n");
		}
	} else {
		emit("%%define __fetch		  __ctx+cpucontext_fetch\n");
		emit("%%define __readbus	  __ctx+cpucontext_readbus\n");
		emit("%%define __writebus	  __ctx+cpucontext_writebus\n");
		emit("%%define __s_fetch	  __ctx+cpucontext_s_fetch\n");
		emit("%%define __s_readbus	  __ctx+cpucontext_s_readbus\n");
		emit("%%define __s_writebus	  __ctx+cpucontext_s_writebus\n");
		emit("%%define __u_fetch	  __ctx+cpucontext_u_fetch\n");
		emit("%%define __u_readbus	  __ctx+cpucontext_u_readbus\n");
		emit("%%define __u_writebus	  __ctx+cpucontext_u_writebus\n");
		emit("%%define __f_readbus	  __ctx+cpucontext_f_readbus\n");
		emit("%%define __f_writebus	  __ctx+cpucontext_f_writebus\n");
	}
	if(cputype >= 68000) {
		emit("%%define __resethandler __ctx+cpucontext_resethandler\n");
		emit("%%define __reg		  __ctx+cpucontext_reg\n");
		emit("%%define __dreg		  __ctx+cpucontext_dreg\n");
		emit("%%define __areg		  __ctx+cpucontext_areg\n");
		emit("%%define __a7			  __ctx+cpucontext_a7\n");
		emit("%%define __asp		  __ctx+cpucontext_asp\n");
		emit("%%define __pc			  __ctx+cpucontext_pc\n");
		emit("%%define __odometer	  __ctx+cpucontext_odometer\n");
		/* Bit 0 of __interrupts = stopped state */
		emit("%%define __interrupts	  __ctx+cpucontext_interrupts\n");
		emit("%%define __sr			  __ctx+cpucontext_sr\n");
	}
	if(cputype >= 68010) {
		emit("%%define __sfc		  __ctx+cpucontext_sfc\n");
		emit("%%define __dfc		  __ctx+cpucontext_dfc\n");
		//?? why not emit(".contextfiller01	 resw 1\n");
		emit("%%define __vbr		  __ctx+cpucontext_vbr\n");
		emit("%%define __bkpthandler  __ctx+cpucontext_bkpthandler\n");
	}
	if(cputype == 68010) {
		emit("%%define __loopmode	  __ctx+cpucontext_loopmode\n");
		//emit(".contextfiller10  resb 3\n");
	}
	if(cputype >= 68020) {
		emit("%%define __xsp		  __ctx+cpucontext_xsp\n");
	}
	
	emit("__cycles_needed		 dd 0\n");
	emit("__cycles_leftover		 dd 0\n");
	emit("__fetch_region_start	 dd 0\n");	// 68k address, Fetch region cache
	...
	*/
	...
	emit("__io_fetchbase		 %s 0\n", x64 ? "dq" : "dd");	//	x86 address
	emit("__io_fetchbased_pc	 %s 0\n", x64 ? "dq" : "dd");	//	x86 address
...
static void airlock_exit(void) {
	emit("mov [__io_cycle_counter RIP],edi\n");
	emit("mov [__io_fetchbase RIP],xBP\n");
	emit("mov [__io_fetchbased_pc RIP],xSI\n");
	emit("push xBX\n");
	emit("push xAX\n");
	if(x64) {
		emit("push r12\n");
...
static void airlock_enter(void) {
	...
	emit("pop xAX\n");
	emit("pop xBX\n");
	emit("mov edi,[__io_cycle_counter RIP]\n");
	emit("mov xBP,[__io_fetchbase RIP]\n");
	emit("mov xSI,[__io_fetchbased_pc RIP]\n");
}

#define airlock_stacksize (x64 ? 4*8 : 2*4)
...
static void force_context_switch(void) {
	emit("sub edi,[__cycles_needed RIP]\n");
	emit("mov dword[__cycles_needed RIP],0\n");
}
...
static void force_trickybit_process(void) {
	emit("inc edi\n");
	emit("add [__cycles_leftover RIP],edi\n");
	emit("or edi,byte -1\n");/* smaller than a mov */
}
...
void align_stack(int bytes, char *reg[]) {
	if(stackalign) {
		if(x64) {
			emit("mov r9,rsp\n");
			emit("and r9,byte 0xF\n");
			emit("sub rsp,byte 16\n");
			emit("sub rsp,r9\n");
			emit("mov [rsp],r9b\n");
		} else {
			if(use_stack) {
...
void restore_stack(int bytes, char *reg[]){
	if(stackalign) {
		if(x64) {
			emit("movzx r9,byte [rsp]\n");
			emit("add rsp,byte 0x10\n");
			emit("add rsp,r9\n");
		} else {
			int offset = 0;
			if(use_stack) {
...
*/
	...
	preserve(rBP | rDI | rSI | rBX | rCX | rDX);
	emit("lea xDI,[__jmptbl RIP]\n");
	emit("lea xSI,[__jmptblcomp RIP]\n");
	if(cputype == 68010) {
		emit("lea xBX,[__looptbl RIP]\n");
	}
	emit("lea xDX,[top RIP]\n");
	if(x64)
		emit("lea r8,[__jmptbl+65536*PTRSIZE wrt rip]\n");
	emit(".decomp:\n");
	emit("lodsd\n");
	emit("mov ecx,eax\n");
	...
	if(x64) emit("cmp rdi,r8\n");
	else	emit("cmp edi,__jmptbl+65536*PTRSIZE\n");
	emit("jne short .decomp\n");

	restore(rBP | rDI | rSI | rBX | rCX | rDX);
	emit("xor eax,eax\n");
	emit("ret\n");
...
**	s680x0exec(cycles)
**
**	Entry: # cycles to execute in EAX(x86), RCX(win64), RDI(amd64)
...
*/
	...
	if(!x64 && use_stack) emit("mov eax,[xSP+4]\n");
	else if(x64)		  emit("mov rax,%s\n", REGARG1);

	/*
	** Check for stopped and double-faulted states.
	*/
	emit("test byte[ctx(interrupts)],1\n");
	emit("jz .notstopped\n");
	emit("test byte[ctx(pc)],1\n");
	emit("jz .notfaulted\n");
	emit("or eax,byte -1\n");
	emit("ret\n");
	emit(".notfaulted:\n");
	emit("add [ctx(odometer)],eax\n");
	emit("mov eax,80000000h\n");
	emit("ret\n");
	emit(".notstopped:\n");

	preserve(rBP | rBX | rCX | rDX | rSI | rDI | r12 | r13);

	if(x64) {
		emit("lea r12,[__jmptbl wrt rip]\n");
		emit("lea r13,[contextbegin wrt rip]\n");
	}
	
	emit("mov [__cycles_needed RIP],eax\n");
	...
	cache_ccr();
	emit("xor xBP,xBP\n");
	emit("mov byte[__execinfo RIP],1\n");
	...
	*/
	...
	emit("add xSI,xBP\n");
	emit("test byte[__execinfo RIP],2\n"); /* Check for PC out of bounds */
	emit("jnz near exec_bounderror\n");

	emit("mov dword[__cycles_leftover RIP],0\n");/* an extra precaution */
	...
	*/
	emit("mov ch,[ctx(sr+1)]\n");
	emit("and ch,80h\n"); /* isolate trace flag */
	emit("mov [__trace_trickybit RIP],ch\n");
...
/*	emit("xor ebx,ebx\n");suffice to say, bits 16-31 should be zero... */
	emit("movzx xBX,word[xSI]\n");
	emit("add xSI,byte 2\n");
	if(x64) emit("jmp qword[r12+rbx*8]\n");
	else	emit("jmp dword[__jmptbl+ebx*4]\n");
	...
	*/
	...
	emit("cmp byte[__trace_trickybit RIP],0\n");
	...
	** running.
	*/
	emit("add edi,[__cycles_leftover RIP]\n");
	emit("mov dword[__cycles_leftover RIP],0\n");
	emit("jns %s execloop\n", x64 ? "near" : "short");
	...
	*/
	...
	emit("mov [__pc],esi\n");
	emit("inc edi\n");
	emit("mov edx,[__cycles_needed RIP]\n");
	emit("sub edx,edi\n");
	emit("add [__odometer],edx\n");
	emit("mov byte[__execinfo RIP],0\n");
	...
	*/
	emit("mov dword[__cycles_needed RIP],0\n");
	emit("mov dword[__io_cycle_counter RIP],-1\n");

	emit("mov eax,ecx\n");/* return code */
	restore(rBP | rBX | rCX | rDX | rSI | rDI | r12 | r13);
	emit("ret\n");
...
*/
	...
	emit("mov eax,1\n");
	emit("test [__execinfo RIP],al\n"); /* Ensure s680x0exec() inactive */
	emit("jnz near .return\n");
	emit("cmp PTR[__s_fetch],0\n");
	emit("je near .return\n");
	emit("dec eax\n");
	emit("mov [__execinfo RIP],al\n");
	emit("sub xAX,byte 16\n");
	emit(".gp:\n");
	emit("mov dword[__reg+64+xAX*4],0\n");
	emit("inc xAX\n");
	emit("jnz short .gp\n");
	emit("mov [__asp],eax\n");
	...
	copy_memory_map("s", x64rax[x64]);	// copy_memory_map("s", "eax");
...
**
**	Entry: EAX(x86), RCX(win64), RDI(amd64)
**			 interrupt level
**		   EDX(x86), RDX(win64), RSI(amd64)
**			 vector (-1 for auto, -2 for spurious)
...
*/
	...
	*/
	...
	emit("shl ah,cl\n");
	restore(rCX);
	emit("test [ctx(interrupts)],ah\n");
	emit("jnz .failure\n");
	/*
	** Commit the given interrupt and vector number.
	*/
	emit("or [ctx(interrupts)],ah\n");
	emit("mov ah,0\n");
	emit("mov [__interrupts+xAX],dl\n");
	emit("and byte[ctx(interrupts)],0FEh\n");
	/*
	...
	*/
	emit("mov edx,[__io_cycle_counter RIP]\n");
	emit("inc edx\n");
	emit("add [__cycles_leftover RIP],edx\n");
	emit("mov dword[__io_cycle_counter RIP],-1\n");
	/*
	** Success (0)
	*/
...
*/
	...
	*/
	emit("test byte[__execinfo RIP],1\n");
	emit("jnz .noflush\n");
	/* Make registers "live" */
	pushad();
	if(x64)
...
**
**	Entry: Address of context in EAX(x86), RCX(win64), RDI(amd64)
**	Exit:  Nothing
*/
	begin_source_proc("GetContext");

	sz = preserve(rDX | rDI);
	if(!x64 && use_stack) emit("mov edi,[xSP+%d]\n", sz + 4);
	else if(!IS_AMD64)	  emit("mov xDI,%s\n", REGARG1);
	emit("%%assign i 0\n");
	emit("%%rep (cpucontext_size / 8)\n");
	if(x64){
		emit("	mov rax,[contextbegin+i+0 wrt rip]\n");
		emit("	mov [rdi+i+0],rax\n");
	} else {
	...
	emit("%%assign i i+8\n");
	emit("%%endrep\n");
	emit("%%if (cpucontext_size %% 8)!=0\n");
	emit("	mov eax,[contextbegin+i+0 RIP]\n");
	emit("	mov [xDI+i+0],eax\n");
	emit("%%endif\n");
	restore(rDX | rDI);
	emit("xor eax,eax\n");
	emit("ret\n");
...
*/
	...
	sz = preserve(rDX | rSI);
	if(!x64 && use_stack) emit("mov esi,[xSP+%d]\n", sz + 4);
	else				  emit("mov xSI,%s\n", REGARG1);
	emit("%%assign i 0\n");
	emit("%%rep (cpucontext_size / 8)\n");
	if(x64) {
		emit("	mov rax,[rsi+i+0]\n");
		emit("	mov [contextbegin+i+0 wrt rip],rax\n");
	} else {
	...
	emit("%%endrep\n");
	emit("%%if (cpucontext_size %% 8)!=0\n");
	emit("	mov eax,[xSI+i+0]\n");
	emit("	mov [contextbegin+i+0 RIP],eax\n");
	emit("%%endif\n");
	restore(rDX | rSI);
	emit("xor eax,eax\n");
	emit("ret\n");
...
**	Entry: Address in EAX(x86), RCX(win64), RDI(amd64)
...
*/
	...
	preserve(rBX | rSI | rDI | rBP | r13); /* DI can be destroyed by rebase */
	if(x64)
		emit("lea r13,[contextbegin wrt rip]\n");
	
	emit("push PTR[__fetch]\n");
	emit("mov xBX,[__s_fetch]\n");
	emit("mov [__fetch],xBX\n");
	/* Must save the fetch region cache as well */
	if(x64) {
		emit("push qword[__fetch_region_start RIP]\n");
	} else {
		emit("push dword[__fetch_region_start]\n");
		emit("push dword[__fetch_region_end]\n");
	}
	/* and __execinfo */
	emit("mov bl,[__execinfo RIP]\n");
	emit("push xBX\n");

	emit("xor ebp,ebp\n");
	emit("mov esi,eax\n");
	emit("and byte[__execinfo RIP],0FDh\n");
	/* Force re-base */
	emit("call basefunction\n");
	emit("test byte[__execinfo RIP],2\n");
	emit("mov eax,-1\n");
	...
	emit(".badfetch:\n");
	emit("pop xBX\n");
	emit("mov [__execinfo RIP],bl\n");
	if(x64) {
		emit("pop qword[__fetch_region_start RIP]\n");
	} else {
		emit("pop dword[__fetch_region_end]\n");
		emit("pop dword[__fetch_region_start]\n");
	}
	emit("pop PTR[__fetch]\n");
	restore(rBX | rSI | rDI | rBP | r13); /* DI can be destroyed by rebase */
	emit("ret\n");
...
*/
	...
	emit("mov eax,[__cycles_needed RIP]\n");
	emit("sub eax,[__io_cycle_counter RIP]\n");
	emit("dec eax\n");				 /* eax = elapsed cycles */
	emit("sub eax,[__cycles_leftover RIP]\n");
	emit("add eax,[ctx(odometer)]\n");	/* add to old __odometer */
	emit("ret\n");
...
*/
	...
	emit("add [ctx(odometer)],eax\n");	/* add to old __odometer */
	/* Clear */
	emit("mov eax,[__io_cycle_counter RIP]\n");
	emit("inc eax\n");
	emit("mov [__cycles_needed RIP],eax\n");
	emit("mov eax,[ctx(odometer)]\n");
	emit("mov dword[ctx(odometer)],0\n");
	emit("ret\n");
...
**	Entry: n in EAX(x86), RCX(win64), RDI(amd64)
**	Exit:  Old odometer value in EAX
*/
...
*/
	...
	emit("mov eax,[__cycles_needed RIP]\n");
	emit("sub [__io_cycle_counter RIP],eax\n");
	emit("xor eax,eax\n");
	emit("mov [__cycles_needed RIP],eax\n");
	emit("ret\n");
...
*/
	...
	emit("ret\n");
	emit(".live:\n");
	emit("mov xAX,[__io_fetchbased_pc RIP]\n");
	emit("sub xAX,[__io_fetchbase RIP]\n");
	emit("ret\n");
...
static void gen_flush_interrupts(void) {
	...
	** is non-maskable. */
	...
	emit("mov [save_01 RIP], dl\n");			// Stef Fix (Gens)

	emit("mov dl,[__interrupts+xDX]\n");
	emit("not cl\n");
...
static void ret_timing(int n) {
	...
	if(!hog) {
		if(x64) emit("jmp near execend\n");
		else	emit("jmp execend\n");
...
static void gen_decode_ext(void) {
	...
	if(cputype <= 68010) {
		emit("push xCX\n");
		emit("movzx edx,word[xSI]\n");
		emit("movsx xCX,dl\n");
		emit("add xSI,byte 2\n");
		emit("shr edx,12\n");
		emit("mov edx,[__reg+xDX*4]\n");
		emit("jc short .long\n");
		emit("movsx xDX,dx\n");
		emit(".long:\n");
		emit("add xDX,xCX\n");
...
void perform_cached_rebase(void) {
	...
	emit("cmp esi,[__fetch_region_start RIP]\n");
	emit("jb short ln%d\n", myline);
	emit("cmp esi,[__fetch_region_end RIP]\n");
	emit("jbe short ln%d\n", myline + 1);
	emit("ln%d:\n", myline);
	emit("call basefunction\n");
	emit("ln%d:\n", myline + 1);
...
static void gen_basefunction(void) {
	...
	*/
	if(addressbits < 32) {
		emit("push xSI\n");
		maskaddress(x64rsi[x64]);
	}
	emit("mov xBP,[ctx(fetch)]\n");
	emit(".check:\n");
	if(!x64)
		emit("db 3Eh\n");
	emit("cmp esi,[xBP + pr_lowaddr]\n");			/* Are we smaller? */
	emit("jb short .next\n");		   /* Yes, go to next address */
	if(!x64)
		emit("db 3Eh\n");
	emit("cmp esi,[xBP + pr_highaddr]\n");		   /* Are we bigger? */
	emit("jbe short .base\n");
	emit(".next:\n");
	if(!x64)
		emit("db 3Eh\n");
	emit("cmp dword [xBP + pr_lowaddr],byte -1\n"); /* End of list? */
	emit("je short .outofrange\n");
	emit("add xBP,byte pr_size\n");			/* To the next structure */
	emit("jmp short .check\n");
	/* Bad news... we jumped out into the weeds. */
	emit(".outofrange:\n");
	if(addressbits < 32) emit("pop xSI\n");
	emit("xor ebp,ebp\n");
	emit("mov dword[__fetch_region_start RIP],-1\n");
	emit("mov dword[__fetch_region_end RIP],ebp\n");
	force_context_switch();
	emit("or byte[__execinfo RIP],2\n");
	emit("ret\n");
	...
	** applicable.
	*/
	emit("mov xBP,[xDX + pr_lowaddr]\n");
	if(addressbits < 32) emit("or ebp,esi\n");
	emit("mov [__fetch_region_start RIP],ebp\n");
	emit("mov ebp,[xDX + pr_highaddr]\n");
	if(addressbits < 32) emit("or ebp,esi\n");
	emit("mov [__fetch_region_end RIP],ebp\n");
	emit("mov xBP,[xDX + pr_offset]\n");
	emit("pop xDX\n");
	if(addressbits < 32) {
		/*
...
static void cx2sr(void){
	...
	emit("test byte[xSP+1],20h\n");
	emit("jz short ln%d\n", myline + 1);
	copy_memory_map("s", x64rcx[x64]);	// copy_memory_map("s", "ecx");
	emit("jmp short ln%d\n", myline);
	emit("ln%d:\n", myline + 1);
	copy_memory_map("u", x64rcx[x64]);	// copy_memory_map("u", "ecx");
	emit("ln%d:\n", myline);
	emit("pop xCX\n");

	/* Step 2: set new PPL / supervisor mode / trace flag */
	emit("mov [ctx(sr+1)],ch\n");
...
static void gen_readbw(int size){
	...
	emit("mov [__access_address RIP],edx\n");
	maskaddress("edx");
	emit("mov xCX,[__read%s]\n", sizename[size]);
	emit("read%c_check:\n",z);
	emit("cmp edx,[xCX + dr_lowaddr]\n"); /* Are we smaller? */
	emit("jb short read%c_next\n",z); /* Yes, go to next address */
	emit("cmp edx,[xCX + dr_highaddr]\n"); /* Are we bigger? */
	emit("jbe short read%c_call\n",z);
	emit("read%c_next:\n",z);
	emit("cmp dword[xCX + dr_size + dr_lowaddr],byte -1\n");/* End of list? */
	emit("je short read%c_outofrange\n",z);
	emit("add xCX,byte dr_size\n");	  /* To the next structure */
	emit("jmp short read%c_check\n",z);

	emit("read%c_outofrange:\n",z);
	emit("or ecx,byte -1\n");
	emit("mov edx,[__access_address RIP]\n");
	emit("ret\n");

	emit("read%c_call:\n",z);
	emit("cmp PTR[xCX + dr_memorycall],byte 0\n");
	emit("jne short read%c_callio\n",z);
	emit("sub edx,[xCX + dr_lowaddr]\n");
	emit("add xDX,[xCX + dr_userdata]\n");
	if(size==1){
		emit("xor xDX,byte 1\n");
		emit("mov cl,[xDX]\n");
	}else{
	...
	*/
	...
	emit("call PTR[%s + dr_memorycall]\n", REGARG2);
	restore_stack(8, x86cx);
	emit("mov ecx,eax\n");
	airlock_enter();
	emit("mov edx,[__access_address RIP]\n");
	emit("ret\n");
...
static void gen_readl(void){
	...
	emit("add edx,byte 2\n");
	emit("cmp edx,[xCX + dr_highaddr]\n"); /* Are we bigger? */
	emit("ja %s readl_split\n", x64 || stackalign ? "near" : "short");
	/* Unsplit version */
	emit("sub edx,[xCX + dr_lowaddr]\n");
	emit("add xDX,[xCX + dr_userdata]\n");
	emit("mov ecx,[xDX-2]\n");
...
static void gen_writebw(int size){
	...
	emit("write%c_next:\n",z);
	emit("cmp dword[xCX + dr_lowaddr],byte -1\n");/* End of list? */
	emit("je %s write%c_end\n", x64 ? "near" : "short", z);
	emit("add xCX,byte dr_size\n");	  /* To the next structure */
	...
	*/
	...
	if(!x64) {
		...
		if(win64) {
			...
			emit("mov rdi,rdx\n");
			emit("xor rsi,rsi\n");
			emit("mov %s,[xSP+%d]\n",x86si[size],airlock_stacksize);
			emit("mov rdx,rcx\n");
		}
		align_stack(0, NULL);
	}
	emit("call PTR[%s + dr_memorycall]\n", REGARG3);
	restore_stack(12, x86cx);
	airlock_enter();
	...
	*/
	...
	if(!x64) {
		...
		if(win64) {
			...
			emit("mov rdi,rdx\n");
			emit("xor rsi,rsi\n");
			emit("mov si,[xSP+%d+PTRSIZE*2]\n",airlock_stacksize+2);
			emit("mov rdx,rcx\n");
		}
		align_stack(0, NULL);
	}
	emit("call PTR[%s + dr_memorycall]\n", REGARG3);
	restore_stack(12, x86ax);
	if(!x64) {
		emit("pop ebx\n");
		emit("pop eax\n");
		emit("add eax,byte 2\n");
...
static void gen_group_12_exception(void) {
	...
	*/
	...
	emit("mov byte[__trace_trickybit RIP],0\n");
...
static void ea_step_precalc(int size, enum eamode mode, int reg) {
	...
	case absw:
		...
		break;
	case absl:
		emit("mov edx,dword[xSI]\n");
		emit("add xSI,byte 4\n");
		emit("rol edx,16\n");
		break;
	case pcdp:
		emit("movsx xDX,word[xSI]\n");
		emit("add xDX,xSI\n");
		emit("sub xDX,xBP\n");
		emit("add xSI,byte 2\n");
		break;
...
static void flick_reg(char*op,int needxf,int affectx,int asl,int rotate){
	...
	if(asl){
		...
		case 'c':/* register shift count */
			...
			emit("dec cl\n");
			emit("jnz short ln%d\n",linenum);linenum++;
			emit("mov [__dreg+xBX*4],%s\n",x86dx[main_size]);
			if(affectx){
				emit("mov cl,ah\n");
				emit("and cl,1\n");
				emit("mov [__xflag RIP],cl\n");
			}
			break;
		case '1':/* immediate shift count ==1 */
			emit("sal %s[__dreg+xBX*4],1\n",
				sizename[main_size]
			);
			flags();
			if(affectx)c2x();
...
static void flick_mem(char*op,int needxf,int affectx,int vf,int rotate){
	...
	if(rotate){
		...
		emit("lahf\n");
		emit("or ah,al\n");
		if(affectx)emit("mov [__xflag RIP],al\n");
		emit("mov al,0\n");
	}else{
		if(vf){
			flags();
...
static void i_dbra(void){
	...
	usereg();
	if(cputype==68010){
		emit("movsx xCX,word[xSI]\n");
		emit("cmp xCX,byte -4\n");
		emit("je short r_loopmode_dbra\n");

		/* Regular DBRA */
		emit("r_regular_dbra:\n");
		emit("sub word[__dreg+xBX*4],byte 1\n");
...
static void bitop(int static_cycles){
	...
	if(main_eamode==dreg){
		...
		if(!main_cc){/* BTST */
			emit("mov edx,1\n");
			emit("shl edx,cl\n");
			emit("test [__dreg+xBX*4],edx\n");
			flag_to_z("z");
		}else{
			emit("mov edx,1\n");
			emit("shl edx,cl\n");
			emit("mov ecx,[__dreg+xBX*4]\n");
			emit("test ecx,edx\n");
			flag_to_z("z");
			switch(main_cc){
...
static void i_link(void){
	usereg();
	emit("mov ecx,[__areg+xBX*4]\n");
	ea_store(4,adec,7);
	emit("mov ecx,[__a7]\n");
	emit("mov [__areg+xBX*4],ecx\n");
	emit("movsx edx,word[xSI]\n");
	emit("add ecx,edx\n");
	emit("mov [__a7],ecx\n");
	emit("add xSI,byte 2\n");
...
static void i_extbw(void){
	usereg();
	emit("movsx cx,byte[__dreg+xBX*4]\n");
	emit("mov [__dreg+xBX*4],cx\n");
	selftest(2);
	flags_v0();
	ret_timing(4);
}
...
static void i_extwl(void){
	usereg();
	emit("movsx ecx,word[__dreg+xBX*4]\n");
	emit("mov [__dreg+xBX*4],ecx\n");
	selftest(4);
	flags_v0();
	ret_timing(4);
...
static void i_negx(void){
	...
	if(main_size==4)cycles=6;
	if(main_eamode==dreg){
		emit("mov cl,[__xflag RIP]\n");
		emit("shr cl,1\n");
		if(main_size==1)emit("mov cl,0\n");
		else emit("mov ecx,0\n");
		emit("sbb %s,[__dreg+xBX*4]\n",x86cx[main_size]);
		emit("mov edx,eax\n");
		flags();c2x();
		adjzero("dh");
		emit("mov [__dreg+xBX*4],%s\n",x86cx[main_size]);
...
static void i_not(void){
	...
	if(main_size==4)cycles=6;
	if(main_eamode==dreg){
		emit("xor %s[__dreg+xBX*4],byte -1\n",
			sizename[main_size]
		);
		flags_v0();
	}else{
...
static void i_exg(void){
	...
	emit("mov edx,[__reg+%d+xBX*4]\n",main_ir);
	emit("mov [__reg+%d],edx\n",(main_reg*4)+main_dr);
	emit("mov [__reg+%d+xBX*4],ecx\n",main_ir);
	/* No loop mode */
	ret_timing(6);
}
...
static void opx_dreg(char*op,void(*adjust)()){
	int cycles;
	usereg();
...
static void opx_adec(char*op,void(*adjust)()){
	int cycles;
	usereg();
...
static void i_addx_dreg(void){opx_dreg("adc",NULL);}
static void i_addx_adec(void){opx_adec("adc",NULL);}
static void i_subx_dreg(void){opx_dreg("sbb",NULL);}
static void i_subx_adec(void){opx_adec("sbb",NULL);}
...
static void i_movec_c_to_r(void){
	...
	emit("ln%d:\n",myline);
	emit("add xSI,byte 2\n");
	emit("mov dword[__reg+xBX*4],ecx\n");
	ret_timing(12);
}
...
static void i_movec_r_to_c(void){
	...
	emit("mov edx,ebx\n");
	emit("shr ebx,12\n");
	emit("mov ecx,dword[__reg+xBX*4]\n");
...
int main(int argc, char **argv) {
	...
	for(i = 1; i < argc; i++) {
		...
		if(*a == '-') {
			...
			} else if(!strcmp("stackalign"	, a)) { stackalign = 1;
			} else if(!strcmp("nostackalign", a)) { stackalign = 0;
			} else if(!strcmp("nohog"		, a)) { hog = 0;
			} else if(!strcmp("hog"			, a)) { hog = 1;
			} else if(!strcmp("x86"			, a)) { x64 = 0;
			} else if(!strcmp("x64"			, a)) { x64 = 1;
			} else if(!strcmp("amd64"		, a)) { win64 = 0;
			} else if(!strcmp("win64"		, a)) { win64 = 1;
	...
	if(stackalign < 0 && x64) stackalign = 1;
	else if(stackalign < 0)	  stackalign = 0;

	if(x64 && win64 == -1) {
		fprintf(stderr, "Specifying -x64 requires either -amd64 or -win64 calling convention\n");
		fprintf(stderr, "see STARDOC.TXT for details\n");
		return 1;
	}

	if(x64 && !use_stack) {
		fprintf(stderr, "X64 architecture only supports stack calling convention\n");
		fprintf(stderr, "see STARDOC.TXT for details\n");
		return 1;
	}
	
	if(x64 && stackalign == 0) {
		fprintf(stderr, "X64 architecture does not support -nostackalign\n");
		fprintf(stderr, "see STARDOC.TXT for details\n");
		return 1;
	}
	...
	*/
	...
	emit("bits %d\n", x64 ? 64 : 32);
	align(x64 ? 8 : 4);
	emit("__jmptbl resb 65536 * PTRSIZE\n");
